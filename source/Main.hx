...
class Main extends Sprite
{
	...
	public static function checkMusic()
	{
		// so apparently it doesnt fully support external music so imma just use them assets just in case
		if (LimeAssets.exists(Paths.music('menu/' + _variables.music)))
		{
			flixel.FlxG.sound.playMusic(Paths.music('menu/' + _variables.music), _variables.mvolume / 100);
			var content:String = LimeAssets.getText(Paths.getPreloadPath('music/menu/${_variables.music}_BPM.txt'));
			Conductor.changeBPM(Std.parseFloat(content));
		}
		else
		{
			flixel.FlxG.sound.playMusic(Paths.music('freakyMenu'), _variables.mvolume / 100);
